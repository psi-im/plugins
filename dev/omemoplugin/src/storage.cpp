...
namespace psiomemo {
  ...
  void Storage::initializeDB(signal_context *signalContext) {
    ...
    if (!_db.exec("PRAGMA table_info(simple_store)").next()) {
      ...
      if (signal_protocol_key_helper_generate_registration_id(&deviceId, 1, signalContext) == SG_SUCCESS) {
        ...
        if (signal_protocol_key_helper_generate_identity_key_pair(&identity_key_pair, signalContext) == SG_SUCCESS) {
          signal_buffer *key_buf = nullptr;

          if (ec_public_key_serialize(&key_buf, ratchet_identity_key_pair_get_public(identity_key_pair)) == SG_SUCCESS) {
            storeValue("own_public_key", toQByteArray(key_buf));
            signal_buffer_bzero_free(key_buf);

            if (ec_private_key_serialize(&key_buf, ratchet_identity_key_pair_get_private(identity_key_pair)) == SG_SUCCESS) {
              storeValue("own_private_key", toQByteArray(key_buf));
              signal_buffer_bzero_free(key_buf);

              uint32_t signed_pre_key_id;
              if (signal_protocol_key_helper_generate_registration_id(&signed_pre_key_id, 1, signalContext) == SG_SUCCESS) {
                session_signed_pre_key *signed_pre_key = nullptr;
                auto timestamp = static_cast<uint64_t>(QDateTime::currentMSecsSinceEpoch());
                if (signal_protocol_key_helper_generate_signed_pre_key(&signed_pre_key, identity_key_pair,
                                                                       signed_pre_key_id, timestamp, signalContext) == SG_SUCCESS) {
  ...
  uint32_t Storage::signedPreKeyid() {
    return lookupValue(this, "signed_pre_key_id").toUInt();
  }
