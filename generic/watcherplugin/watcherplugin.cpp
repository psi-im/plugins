...
Watcher::Watcher() :
    psiOptions(nullptr), popup(nullptr), icoHost(nullptr), appInfoHost(nullptr), activeTab(nullptr),
    contactInfo(nullptr), accInfo(nullptr), sound_(nullptr), enabled(false), soundFile("sound/watcher.wav")
    //, Interval(2)
    ,
    model_(nullptr), isSndEnable(false), disableSnd(true), disablePopupDnd(true), popupId(0)
{
}
...
bool Watcher::enable()
{
    if (psiOptions) {
        ...
        for (const QString &settings : list) {
        ...
        if (stanza.tagName() == "presence") {
            ...
            if (find) {
                ...
                if (model_->statusByJid(from) != status) {
                    ...
                    QMetaObject::invokeMethod(this, "showPopup", Qt::QueuedConnection, Q_ARG(int, acc),
                                              Q_ARG(const QString &, bare), Q_ARG(QString, text));
                }
            }
...
bool Watcher::outgoingStanza(int /*account*/, QDomElement & /*xml*/) { return false; }
    ...
    if (!wi->jid().isEmpty() && from.contains(QRegExp(wi->jid(), Qt::CaseInsensitive, QRegExp::Wildcard))) {
        ...
        if (wi->alwaysUse() || isSndEnable) {
            ...
            QTimer::singleShot(500, this,
                               SLOT(timeOut())); // включаем все звуки через секунду, чтобы не
                                                 // игралось два звука одновременно
            return true;
        }
    }
...
void Watcher::setPopupAccessingHost(PopupAccessingHost *host) { popup = host; }

void Watcher::setIconFactoryAccessingHost(IconFactoryAccessingHost *host) { icoHost = host; }

void Watcher::setActiveTabAccessingHost(ActiveTabAccessingHost *host) { activeTab = host; }

void Watcher::setOptionAccessingHost(OptionAccessingHost *host) { psiOptions = host; }

void Watcher::setApplicationInfoAccessingHost(ApplicationInfoAccessingHost *host) { appInfoHost = host; }

void Watcher::setContactInfoAccessingHost(ContactInfoAccessingHost *host) { contactInfo = host; }

void Watcher::optionChanged(const QString &option) { Q_UNUSED(option); }

void Watcher::setAccountInfoAccessingHost(AccountInfoAccessingHost *host) { accInfo = host; }

void Watcher::setSoundAccessingHost(SoundAccessingHost *host) { sound_ = host; }
...
QAction *Watcher::createAction(QObject *parent, const QString &contact)
{
    ...
    if (jids.contains(contact, Qt::CaseInsensitive) && model_->jidEnabled(contact)) {
        ...
        action->setProperty("watch", QVariant(false));
    }
...
void Watcher::removeFromActions(QObject *object) { actions_.remove(object->property("jid").toString()); }

void Watcher::playSound(const QString &f) { sound_->playSound(f); }

void Watcher::getSound(QModelIndex index)
{
    if (ui_.tb_open->isDown()) {
        QString fileName = QFileDialog::getOpenFileName(
            nullptr, tr("Choose a sound file"), psiOptions->getPluginOption(constLastFile, QVariant("")).toString(),
            tr("Sound (*.wav)"));
        if (fileName.isEmpty())
            return;
...
void Watcher::onOptionsClose() { model_->reset(); }

QList<QVariantHash> Watcher::getAccountMenuParam() { return QList<QVariantHash>(); }

QList<QVariantHash> Watcher::getContactMenuParam() { return QList<QVariantHash>(); }
...
{
    return tr("This plugin is designed to monitor the status of specific roster "
              "contacts, as well as for substitution of standard sounds of "
              "incoming messages.\n"
              "On the first tab set up a list of contacts for the status of "
              "which is monitored. When the status of such contacts changes a "
              "popup window will be shown"
              " and when the status changes to online a custom sound can be "
              "played."
              "On the second tab is configured list of items, the messages are "
              "being monitored. Each element can contain a regular expression"
              " to check for matches with JID, from which the message arrives, a "
              "list of regular expressions to check for matches with the text"
              " of an incoming message, the path to sound file which will be "
              "played in case of coincidence, as well as the setting, whether "
              "the sound"
