...
bool Reminder::outgoingStanza(int /*account*/, QDomElement & /*xml*/) { return false; }

void Reminder::setAccountInfoAccessingHost(AccountInfoAccessingHost *host) { accInfoHost = host; }

void Reminder::setApplicationInfoAccessingHost(ApplicationInfoAccessingHost *host) { appInfoHost = host; }
...
void Reminder::setStanzaSendingHost(StanzaSendingHost *host) { stanzaHost = host; }
...
void Reminder::updateVCard()
{
    if (enabled && !updateInProgress) {
        ...
        QTimer::singleShot(30000, this,
                           SLOT(timeoutStopUpdate())); // 30 секунд дольжно хватить, чтобы получить все vCard'ы
    }
}

void Reminder::timeoutStopUpdate() { updateInProgress = false; }
            ...
            if (file.open(QIODevice::ReadOnly)) {
                ...
                if (current != Birthday) {
                    ...
                    if (!checkFromRoster || Roster_.contains(jid)) {
                        if (daysTo == 0) {
                            CheckResult += Nick + " (" + jid + ") "
                                + tr("celebrates birthday today!"
                                     "\n");
                        } else if (daysTo <= days_ && daysTo > 0) {
                            CheckResult
                                += Nick + " (" + jid + ") " + tr("celebrates birthday in %n day(s)\n", "", daysTo);
                        } else if (daysTo == -1) {
...
void Reminder::setOptionAccessingHost(OptionAccessingHost *host) { psiOptions = host; }

void Reminder::setIconFactoryAccessingHost(IconFactoryAccessingHost *host) { icoHost = host; }

void Reminder::setSoundAccessingHost(SoundAccessingHost *host) { sound_ = host; }

void Reminder::setContactInfoAccessingHost(ContactInfoAccessingHost *host) { contactInfo = host; }
...
void Reminder::setPopupAccessingHost(PopupAccessingHost *host) { popup = host; }

void Reminder::playSound(const QString &f) { sound_->playSound(f); }
...
void Reminder::checkSound() { playSound(ui_.le_sound->text()); }
