...
Model::Model(const QStringList &Jids_, QVariantList Sounds_, QObject *parent) : QAbstractTableModel(parent), Jids(Jids_)
{
...
int Model::rowCount(const QModelIndex & /* parent*/) const { return tmpJids_.size(); }
...
void Model::reset() { tmpJids_ = Jids; }
...
void Model::apply() { Jids = tmpJids_; }

int Model::indexByJid(const QString &jid) const { return Jids.indexOf(jid); }

QStringList Model::getJids() const { return Jids; }
