...
class GetTextDlg : public QDialog {
    Q_OBJECT
public:
    GetTextDlg(QWidget *parent) : QDialog(parent)
    {
...
//---SelectionRect--------------
//------------------------------
SelectionRect::SelectionRect() : QRect() { }

SelectionRect::SelectionRect(int left, int top, int w, int h) : QRect(left, top, w, h) { }
...
SelectionRect::CornerType SelectionRect::cornerUnderMouse(const QPoint &pos) const
{
    ...
    if (qAbs(topLeft().x() - pos.x()) < ACCURACY && qAbs(topLeft().y() - pos.y()) < ACCURACY) {
        return TopLeft;
    } else if (qAbs(topRight().x() - pos.x()) < ACCURACY && qAbs(topRight().y() - pos.y()) < ACCURACY) {
        return TopRight;
    } else if (qAbs(bottomLeft().x() - pos.x()) < ACCURACY && qAbs(bottomLeft().y() - pos.y()) < ACCURACY) {
        return BottomLeft;
    } else if (qAbs(bottomRight().x() - pos.x()) < ACCURACY && qAbs(bottomRight().y() - pos.y()) < ACCURACY) {
        return BottomRight;
    }
...
PixmapWidget::PixmapWidget(QWidget *parent) :
    QWidget(parent), bar_(nullptr), type_(ToolBar::ButtonNoButton), p1(QPoint(-1, -1)), p2(QPoint(-1, -1)),
    selectionRect(new SelectionRect(-1, -1, -1, -1)), currentCursor(QCursor(Qt::CrossCursor)),
    cornerType(SelectionRect::NoCorner), smoothLineType_(None)
{
...
void PixmapWidget::setToolBar(ToolBar *bar)
{
    ...
    connect(bar_, SIGNAL(checkedButtonChanged(ToolBar::ButtonType)), this,
            SLOT(checkedButtonChanged(ToolBar::ButtonType)));
    connect(bar_, SIGNAL(newWidth(int)), this, SLOT(newWidth(int)));
    bar_->checkButton(ToolBar::ButtonSelect);

    Options *o = Options::instance();
    init(o->getOption(constPenWidth, 2).toInt(), o->getOption(constColor, "#000000").toString(),
...
{
    if (selectionRect->contains(e->pos()) && e->buttons() == Qt::LeftButton)
        cut();
...
void PixmapWidget::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    if (type_ == ToolBar::ButtonPen) {
        ...
        selectionRect->setCoords(qMin(p1.x(), p2.x()), qMin(p1.y(), p2.y()), qMax(p1.x(), p2.x()),
                                 qMax(p1.y(), p2.y()));
        GetTextDlg gtd(this);
