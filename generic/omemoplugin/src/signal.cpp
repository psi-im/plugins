...
uint32_t Signal::getDeviceId() { return m_deviceId; }
...
void Signal::generatePreKeys()
{
    ...
    if (actualCount < preKeyCount) {
        ...
        if (signal_protocol_key_helper_generate_pre_keys(&pre_keys_head, startId, count, m_signalContext)
            == SG_SUCCESS) {
...
Bundle Signal::collectBundle()
{
    ...
    if (signal_protocol_signed_pre_key_load_key(m_storage.storeContext(), &signed_pre_key, bundle.signedPreKeyId)
        != SG_SUCCESS) {
        return bundle;
    }
        ...
        if (signed_pre_key_public != nullptr) {
            ...
            if (identity_key != nullptr) {
                ...
                if (session_pre_key_bundle_create(
                        &pre_key_bundle, deviceId, 0, preKey.first, pre_key_public, bundle.signedPreKeyId,
...
ec_public_key *Signal::curveDecodePoint(const QByteArray &bytes) const
{
    ...
    curve_decode_point(&publicKey, reinterpret_cast<const uint8_t *>(bytes.data()), static_cast<size_t>(bytes.size()),
                       m_signalContext);
    return publicKey;
}
...
QPair<QByteArray, bool> Signal::decryptKey(const QString &sender, const EncryptedKey &encryptedKey)
{
    ...
    if (encryptedKey.isPreKey) {
        session_builder *session_builder = nullptr;
        if (session_builder_create(&session_builder, m_storage.storeContext(), &sender_addr, m_signalContext)
            == SG_SUCCESS) {
            doWithCipher(
                &sender_addr, encryptedKey.key, [&](session_cipher *cipher, const uint8_t *keyData, size_t keyLen) {
                    ...
                    if (pre_key_signal_message_deserialize(&message, keyData, keyLen, m_signalContext) == SG_SUCCESS) {
                        ...
                        int            result
                            = session_cipher_decrypt_pre_key_signal_message(cipher, message, nullptr, &plaintext);
...
bool Signal::isTrusted(const QString &user, uint32_t deviceId) { return m_storage.isTrusted(user, deviceId); }

template <typename T> void Signal::doWithCipher(signal_protocol_address *addr, const QByteArray &key, T &&lambda)
{
...
uint32_t Signal::preKeyCount() { return m_storage.preKeyCount(); }
...
void Signal::setEnabledForUser(const QString &user, bool enabled) { m_storage.setEnabledForUser(user, enabled); }
...
bool Bundle::isValid()
{
    return !signedPreKeyPublic.isNull() && !signedPreKeySignature.isNull() && !identityKeyPublic.isNull()
        && !preKeys.isEmpty();
}
}
