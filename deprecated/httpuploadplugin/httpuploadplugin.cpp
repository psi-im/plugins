...
QString escape(const QString &plain) { return plain.toHtmlEscaped(); }
...
class HttpUploadPlugin : public QObject,
    ...
    Q_INTERFACES(PsiPlugin ToolbarIconAccessor GCToolbarIconAccessor StanzaSender ActiveTabAccessor PsiAccountController
                     OptionAccessor IconFactoryAccessor AccountInfoAccessor PluginInfoProvider ChatTabAccessor
...
public:
    ...
    virtual void setupChatTab(QWidget *, int account, const QString &) { checkUploadAvailability(account); }
    virtual void setupGCTab(QWidget *, int account, const QString &) { checkUploadAvailability(account); }
    ...
    QString      getId(int account) { return stanzaSender->uniqueId(account); }
...
HttpUploadPlugin::HttpUploadPlugin() :
    ...
    psiOptions(nullptr), appInfoHost(nullptr), enabled(false), manager(new QNetworkAccessManager(this)),
    sb_previewWidth(nullptr), cb_resize(nullptr), sb_size(nullptr), sb_quality(nullptr), imageResize(false),
    imageSize(0), imageQuality(0), previewWidth(0)
{
...
QString HttpUploadPlugin::name() const { return "HTTP Upload Plugin"; }

QString HttpUploadPlugin::shortName() const { return "httpupload"; }
QString HttpUploadPlugin::version() const { return constVersion; }
...
QList<QVariantHash> HttpUploadPlugin::getGCButtonParam() { return getButtonParam(); }

void HttpUploadPlugin::setAccountInfoAccessingHost(AccountInfoAccessingHost *host) { accInfo = host; }

void HttpUploadPlugin::setIconFactoryAccessingHost(IconFactoryAccessingHost *host) { iconHost = host; }

void HttpUploadPlugin::setPsiAccountControllingHost(PsiAccountControllingHost *host) { psiController = host; }

void HttpUploadPlugin::setOptionAccessingHost(OptionAccessingHost *host) { psiOptions = host; }

void HttpUploadPlugin::setPluginAccessingHost(PluginAccessingHost *host) { pluginHost = host; }

void HttpUploadPlugin::setStanzaSendingHost(StanzaSendingHost *host) { stanzaSender = host; }

void HttpUploadPlugin::setActiveTabAccessingHost(ActiveTabAccessingHost *host) { activeTab = host; }

void HttpUploadPlugin::uploadFile() { upload(true); }

void HttpUploadPlugin::uploadImage() { upload(false); }
...
void HttpUploadPlugin::upload(bool anything)
{
    ...
    if (!dataSource.isNull()) {
        QMessageBox::warning(
            nullptr, tr("Please wait"),
            tr("Another upload operation is already in progress. Please wait up to %1 sec for it to complete or fail.")
                .arg(SLOT_TIMEOUT / 1000));
        return;
    }
    QString                                serviceName;
    int                                    sizeLimit = -1;
    ...
    if (!anything && imageResize
        ...
        pix.scaled(imageSize, imageSize, Qt::KeepAspectRatio, Qt::SmoothTransformation)
            .save(&buffer, type.toLatin1().constData(), imageQuality);
#ifdef DEBUG_UPLOAD
...
void HttpUploadPlugin::uploadComplete(QNetworkReply *reply)
{
    ...
    if (ok && (statusCode == 201 || statusCode == 200)) {
        ...
        if (omemoEncrypted) {
            ...
            if (message.isNull()) {
                QMessageBox::critical(nullptr, tr("Error uploading"),
                                      tr("Something has gone wrong, please try again or disable OMEMO encryption"));
                return;
            }
        }
        ...
        if (currentUpload.type == "chat") {
            ...
            if (omemoEncrypted) {
                psiController->appendMsg(currentUpload.account, currentUpload.to,
                                         QUrl::fromLocalFile(currentUpload.localFilePath).toString(QUrl::FullyEncoded),
                                         id, omemoEncrypted);
            }
        }
    } else {
        QMessageBox::critical(nullptr, tr("Error uploading"),
                              tr("Upload error %1; HTTP code %2, message: %3")
...
void HttpUploadPlugin::setApplicationInfoAccessingHost(ApplicationInfoAccessingHost *host) { appInfoHost = host; }
...
void HttpUploadPlugin::handleSslError(QNetworkReply *reply, const QList<QSslError> &) { reply->ignoreSslErrors(); }
